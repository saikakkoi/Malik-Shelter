  ...
  <div class="max-w-6xl mx-auto p-6 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
    ...
    <div class="flex items-center gap-4">
      <button @click="router.back()" class="bg-primary/10 p-3 rounded-2xl hover:bg-primary/20 transition-colors group">
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-accent group-hover:-translate-x-1 transition-transform"><path d="m15 18-6-6 6-6"/></svg>
      </button>
      <div>
        <h1 class="text-4xl text-accent font-bold">{{ isEdit ? 'Update Friend' : 'New Animal Arrival' }}</h1>
        <p class="text-muted text-lg">{{ isEdit ? 'Refresh their profile details' : 'Build a heartwarming profile for a new friend' }}</p>
      </div>
    </div>
      ...
      <aside class="hidden lg:block lg:w-64 lg:sticky lg:top-8 space-y-6">
        ...
        <div class="bg-primary/5 rounded-[32px] p-6 border border-primary/10">
          ...
          <p class="text-xs text-muted leading-relaxed">
            Detailed bios and clear photos increase adoption rates by <span class="text-accent font-bold">40%</span>. Take your time!
          </p>
        </div>
      </aside>
      ...
      <form @submit.prevent="handleSubmit" class="flex-1 space-y-12 pb-32">
        ...
        >
          <div class="flex items-center gap-3 border-b border-primary/10 pb-4">
            ...
            <span class="ml-auto text-xs font-bold text-muted uppercase tracking-widest">{{ formData.photos.length }} / {{ MAX_PHOTOS }}</span>
          </div>

          <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            <!-- Upload Button -->
            <label 
              v-if="formData.photos.length < MAX_PHOTOS"
            ...
            >
              ...
              <div class="w-10 h-10 bg-primary/10 rounded-full flex items-center justify-center text-xl group-hover:bg-accent group-hover:text-white transition-colors">+</div>
            ...
            >
              ...
              <!-- Main Badge -->
              <div v-if="photo.is_main" class="absolute top-2 left-2 bg-accent text-white text-[8px] font-bold px-2 py-0.5 rounded-full uppercase tracking-widest shadow-sm">
                Main
              </div>

              <!-- Overlay Controls -->
              <div class="absolute inset-0 bg-dark/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2 px-2">
                <button 
                  v-if="!photo.is_main"
                  type="button"
          ...
          <p class="text-[10px] text-muted ml-1 italic">* Max 5 photos, 5MB each. First photo is set as main by default.</p>
        </section>
          ...
          <div class="space-y-6">
            <div class="space-y-2">
              ...
              <textarea 
                ...
                class="w-full bg-warm-white border-2 border-primary/20 rounded-2xl px-5 py-4 focus:border-accent focus:ring-0 transition-colors font-body resize-none"
              ></textarea>
            </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="space-y-4">
              <div class="flex items-center justify-between p-4 bg-warm-white rounded-2xl border-2 border-primary/20">
                ...
                <button 
                  type="button"
                  @click="formData.is_sterilized = !formData.is_sterilized"
                ...
                >
                  <div 
                    ...
                    :class="formData.is_sterilized ? 'left-7' : 'left-1'"
                  ></div>
                </button>
              </div>
              ...
              <div class="space-y-2">
                ...
                <input 
                  ...
                  type="text" 
                  placeholder="e.g. Grain-free diet"
                  class="w-full bg-warm-white border-2 border-primary/20 rounded-2xl px-5 py-3 focus:border-accent focus:ring-0 transition-colors font-body"
                />
              </div>
            </div>
            ...
            <div class="space-y-6">
              <div class="space-y-3">
                ...
                <div class="flex gap-2">
                  <button 
                    v-for="lvl in energyOptions" 
                    :key="lvl"
                    type="button"
                    @click="formData.energy_level = lvl"
              ...
              <div class="grid grid-cols-2 gap-3">
                ...
                >
                  <input 
                    type="checkbox" 
                    :value="opt"
                    v-model="formData.social_compatibility"
    ...
    <div class="fixed bottom-6 left-1/2 -translate-x-1/2 w-full max-w-6xl px-6 pointer-events-none z-50">
      <div class="bg-white/80 backdrop-blur-md rounded-[32px] p-4 shadow-xl border border-primary/10 flex justify-between items-center pointer-events-auto">
        <button 
          type="button"
          @click="router.back()"
